...
export const validationSchema = yup.object({
  swimming_class_customer_history_id: yup
    .number()
    .required("swimming_class_customer_history_id wajib disertakan")
    .test({
      name: "is-id-exists",
      skipAbsent: true,
      async test(id) {
        ...
        const swimmingClassCustomerHistory =
          await db.query.swimmingClassCustomerHistories.findFirst({
            where: (swimmingClassCustomerHistories, { eq }) =>
              eq(swimmingClassCustomerHistories.id, BigInt(id)),
          });

        if (!swimmingClassCustomerHistory) return false;

        return true;
      },
      message:
        "Data riwayat kelas renang dari ID yang Dikirimkan Tidak Ditemukan",
    }),
  payments: yup
    .array()
