...
import { getGazeboTypeWithProductCode } from "@/lib/enums/GazeboTypeEnum";
...
import { SalesGazeboRentPromoTypeEnum } from "@/lib/enums/SalesGazeboRentPromoTypeEnum";
...
const storeSalesValidationSchema = yup.object({
  ...
  gazebo_type_id: yup
    .number()
    .required("Tipe gazebo diisi")
    .typeError("Tipe gazebo diisi dengan karakter yang valid"),
  gazebo_type_name: yup
    .string()
    .required("Nama tipe gezebo diisi")
    .typeError("Nama tipe gezebo diisi dengan karakter yang valid"),
  gazebo_price: yup
    .number()
    .required("Harga tipe gezebo diisi")
    .typeError("Harga tipe gezebo diisi dengan karakter yang valid"),
  promo_type: yup
    .string()
  ...
  promo_code: yup
    .string()
    .when("promo_type", {
      is: SalesGazeboRentPromoTypeEnum.PROMO,
  ...
  booklet_code: yup
    .string()
    .when("promo_type", {
      is: SalesGazeboRentPromoTypeEnum.BOOKLET,
...
export default async function storeSales(
  ...
  try {
    ...
    const isAuthorized = await can({
      permissionNames: [PermissionEnum.CASHQ_TRANSACTION_GAZEBO_RENT_STORE],
      user: user,
    ...
    const validatedValues = await storeSalesValidationSchema.validate(
      {
        wallet_id: walletId,
        gazebo_type_id: formData.get("gazebo_type_id"),
        gazebo_type_name: formData.get("gazebo_type_name"),
        gazebo_price: formData.get("gazebo_price"),
    ...
    if (validatedValues.promo_type === SalesGazeboRentPromoTypeEnum.PROMO)
      promoCode = validatedValues.promo_code;
    else if (
      validatedValues.promo_type === SalesGazeboRentPromoTypeEnum.BOOKLET
    ) {
    ...
    const recomposeSalesTemporary = await getRecomposeSalesTemporary({
      sales_details: [
        {
          price: validatedValues.gazebo_price,
          product_id: validatedValues.gazebo_type_id,
          qty: 1,
        },
      ],
    });
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.insert(salesGazeboRent).values({
        sales_id: salesId,
        promo_type: validatedValues.promo_type,
      });

      // Store data to booklet used promo table
      if (validatedValues.promo_type === SalesGazeboRentPromoTypeEnum.BOOKLET) {
        const { promo } = await getValidPromo(promoCode ?? "");
      ...
      const { walletHistoryId } = await storePayment({
        ...
        paymentSummary: {
          ...
          wallet_transaction_type: WalletTransactionTypeEnum.GAZEBO_RENT,
        },
      });

      await tx.insert(gazeboWallet).values({
        wallet_id: BigInt(validatedValues.wallet_id),
        wallet_history_id: walletHistoryId!,
        type: getGazeboTypeWithProductCode(sales.salesDetails[0].product.code),
        payment_status: GazeboWalletPaymentStatusEnum.PAID,
        created_by: BigInt(user.id),
      });
    });
    ...
    return {
      ...
      url: `/cashq-transaction/${walletId}/gazebo-rent/sales/${salesId}/receipt`,
    };
  } catch (error: any) {
    console.error(error);
