...
export default async function GazeboRentSalesReceiptPage({
  params,
}: {
  ...
  await can({
    permissionNames: [PermissionEnum.CASHQ_TRANSACTION_GAZEBO_RENT_RECEIPT],
    user: user,
  });
  ...
  const latestGazeboWallet = wallet.gazeboWallets.pop();

  if (!latestGazeboWallet) return notFound();
    ...
    <Card className="lg:w-1/2 xl:w-2/6">
      <CardContent className="flex w-full flex-col justify-center gap-4 p-4">
        ...
        <ButtonPrintReceipt
          ...
          redirectUrl={`/cashq-transaction/${walletId}/gazebo-rent/${latestGazeboWallet.id}/edit`}
        />
      </CardContent>
    </Card>
  );
