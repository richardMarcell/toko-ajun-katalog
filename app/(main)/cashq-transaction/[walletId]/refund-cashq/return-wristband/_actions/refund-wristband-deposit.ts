...
export default async function refundWristbandDeposit({
  ...
  try {
    ...
    const validWristbandCode = walletWristbandHasReturnedList.map(
      ({ wristband_code }) => wristband_code,
    );

    const isRequestedWristbandCodeValid =
      wristbandCodes.length === validWristbandCode.length &&
      wristbandCodes.every((val, index) => val === validWristbandCode[index]);

    if (!isRequestedWristbandCodeValid)
      throw new Error("Wristband code is not valid", {
        cause:
          "Terdapat kesalahan pada input kode gelang. Silakan periksa kembali kode gelang yang Anda masukkan",
      });

    const totalDepositRefund =
      walletWristbandHasReturnedList.length * Number(wristbandProduct.price);

    await db.transaction(async (tx) => {
      const { walletCashRefundId } = await storeWalletCashRefund({
        totalRefund: totalDepositRefund.toString(),
        refundDetails: walletWristbandHasReturnedList.map((walletWristband) => {
          return {
    ...
    return {
      ...
      url: `/cashq-transaction/${wallet.id}/refund-cashq/receipt-deposit-withdraw-only?wristbandCodes=${wristbandCodes.join(",")}`,
    };
  } catch (error: any) {
    console.error(error);
