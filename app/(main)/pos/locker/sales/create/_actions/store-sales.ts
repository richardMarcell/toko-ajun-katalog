...
import { salesLocker } from "@/db/schema/sales-locker";
...
import { SalesTemporary } from "@/types/domains/pos/locker/sales/create";
...
export async function storeSales(
  ...
  try {
    ...
    const permissionNames = [PermissionEnum.LOCKER_SALES_STORE];
    ...
    await db.transaction(async (tx) => {
      const sales = await proccessSalesTransaction({
        ...
        transactionType: SalesTransactionTypeEnum.LOCKER_SOUVENIR_SALE,
        tx: tx,
        promoCode: validatedValues.promo_code,
      });
      salesId = sales.id;

      // Insert data to table sales locker
      await storeSalesLocker(validatedValues, tx, salesId);
    ...
    return {
      ...
      url: `/pos/locker/sales/${Number(salesId)}/receipt`,
    };
  } catch (error: any) {
    console.error(error);
...
async function storeSalesLocker(
  validatedValues: ValidatedValues,
  tx: DatabaseTransaction,
  salesId: bigint,
): Promise<void> {
  await tx.insert(salesLocker).values({
    sales_id: salesId,
  ...
  for (const payment of validatedValues.payments) {
    ...
    await storePayment({
      ...
      paymentSummary: {
        ...
        wallet_transaction_type: WalletTransactionTypeEnum.LOCKER_SOUVENIR_SALE,
