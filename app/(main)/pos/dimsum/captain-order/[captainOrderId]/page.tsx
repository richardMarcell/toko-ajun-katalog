...
export default async function CaptainOrderDetailPage({
  ...
  await can({
    permissionNames: [PermissionEnum.DIMSUM_CAPTAIN_ORDER_SHOW],
    user: user,
  });
    ...
    <div className="space-y-4">
      <div className="mt-4 grid w-full grid-cols-2 rounded-md border bg-white p-4">
        ...
        <Table>
          <TableBody>
            <TableRow className="border-none">
              <TableCell className="w-52 font-bold">No. Meja</TableCell>
              <TableCell className="w-4">:</TableCell>
              <TableCell>
                {captainOrder.table ? captainOrder.table.name : "-"}
              </TableCell>
            </TableRow>
            <TableRow className="border-none">
        ...
        <div className="flex gap-4 pt-4">
          ...
          <div className="flex gap-2">
            <Button asChild variant={"outline"}>
              <Link href={`/pos/dimsum/captain-order`}>Kembali</Link>
            </Button>
          </div>
        </div>
      </div>
