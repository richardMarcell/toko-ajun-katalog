...
import { storeDimsumSalesTemporary } from "../_actions/store-dimsum-sales-temporary";
import {
...
export function DialogConfirmationPaidItem({
  ...
  const [state, formAction] = useActionState(
    storeDimsumSalesTemporary,
    initialValue,
  );

  const [dimsumSalesTemporary, setDimsumSalesTemporary] =
    useState<DimsumSalesTemporary>({
  ...
  const handleButtonPayOnClick = () => {
    startTransition(() => {
      ...
      formData.set(
        "sales_details",
        JSON.stringify(dimsumSalesTemporary.sales_details),
      );
        ...
        <ScrollArea className="h-[500px]">
          <div className="space-y-8">
            {captainOrder.captainOrderDetails.map((detail) => (
              <CardProduct
                dimsumSalesTemporary={dimsumSalesTemporary}
                setDimsumSalesTemporary={setDimsumSalesTemporary}
...
function CardProduct({
  captainOrderDetail,
  dimsumSalesTemporary,
  setDimsumSalesTemporary,
}: {
  captainOrderDetail: CaptainOrderDetailIncludeRelationship;
  dimsumSalesTemporary: DimsumSalesTemporary;
  setDimsumSalesTemporary: Dispatch<SetStateAction<DimsumSalesTemporary>>;
}) {
  ...
  const orderedProduct = dimsumSalesTemporary.sales_details.find(
  ...
  const handleProductQtyChange = (changeQty: number) => {
    const orderedProducts = [...dimsumSalesTemporary.sales_details];
