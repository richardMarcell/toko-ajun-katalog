...
import { getCaptainOrder } from "@/app/(main)/pos/dimsum/_repositories/get-captain-order";
import { db } from "@/db";
...
type ValidatedValues = {
  dimsum_sales_temporary: DimsumSalesTemporary;
  payments: {
...
export async function storeSales(
  ...
  try {
    ...
    const permissionNames = [PermissionEnum.DIMSUM_SALES_STORE];
    ...
    const captainOrderIdValidated =
      validatedValues.dimsum_sales_temporary.captain_order_id.toString();
    ...
    const recomposeSalesTemporary = await getRecomposeSalesTemporary(
      validatedValues.dimsum_sales_temporary,
    );
    ...
    await db.transaction(async (tx) => {
      const dimsumSalesTemporaryOrigin = validatedValues.dimsum_sales_temporary;
      const specialItemId = ProductConfig.special_item.id;

      const dimsumSalesTemporary = {
        ...dimsumSalesTemporaryOrigin,
        sales_details: dimsumSalesTemporaryOrigin.sales_details.map(
          (detail) => {
      ...
      const sales = await proccessSalesTransaction({
        salesTemporary: dimsumSalesTemporary,
        sateliteConfig: SateliteUnitConfig.dimsum,
        transactionType: SalesTransactionTypeEnum.DIMSUM_SALE,
        tx: tx,
    ...
    return {
      ...
      url: `/pos/dimsum/captain-order/${captainOrder.id}/sales/${salesId}/receipt`,
    };
  } catch (error: any) {
    console.error(error);
...
async function validateRequest(formData: FormData): Promise<ValidatedValues> {
  const validatedValues = await storeSalesValidationSchema.validate(
    {
      dimsum_sales_temporary: JSON.parse(
        formData.get("dimsum_sales_temporary") as string,
      ),
