...
export async function storeSalesTemporary(
  ...
  await can({
    permissionNames: [PermissionEnum.SOUVENIR_SALES_STORE],
    user: user,
  });
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      const salesTemporaryData = {
        ...
        unit_business: SateliteUnitConfig.souvenir.unit_business,
        sales_details: salesTemporaryWithTax.sales_details,
        customer_name:
          validatedValues.customer_name == ""
            ? null
            : validatedValues.customer_name,
      };
  ...
  redirect("/pos/souvenir/sales/create", RedirectType.push);
}
