...
export async function storeCaptainOrder(
  ...
  await can({
    permissionNames: [PermissionEnum.RESTO_PATIO_CAPTAIN_ORDER_STORE],
    user: user,
  });
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      const isPatioBistro =
        validatedValues.outlet === CaptainOrderOutleEnum.PATIO_BISTRO;
      const isRoomServices =
        validatedValues.outlet === CaptainOrderOutleEnum.ROOM_SERVICES;
      const tableId =
        isPatioBistro && validatedValues.table_id
          ? BigInt(validatedValues.table_id)
          : null;
      const roomId =
        isRoomServices && validatedValues.room_id
          ? BigInt(validatedValues.room_id)
          : null;
    ...
    return {
      ...
      url: `/pos/resto/patio/captain-order/${captainOrderId}/edit`,
    };
  } catch (error: any) {
    console.error(error);
