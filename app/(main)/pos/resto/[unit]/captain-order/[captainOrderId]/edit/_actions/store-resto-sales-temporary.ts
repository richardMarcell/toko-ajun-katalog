...
import { storeRestoSalesTemporaryValidationSchema } from "./_libs/store-resto-sales-temporary-validation-schema";

/* eslint-disable @typescript-eslint/no-explicit-any */
export async function storeRestoSalesTemporary(
  prevState: any,
  formData: FormData,
  ...
  try {
    ...
    const salesTemporaryWithTax = await getSalesTemporaryWithTax(
      ...
      SateliteUnitConfig.resto_patio.unit_business,
    );
    ...
    return {
      ...
      data: salesTemporary,
      url: `/pos/resto/patio/captain-order/${validatedValues.captain_order_id}/sales/create`,
    };
  } catch (error: any) {
    console.error(error);
...
async function validateRequest(formData: FormData) {
  const validatedValues =
    await storeRestoSalesTemporaryValidationSchema.validate(
      {
